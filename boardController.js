...
exports.getList  = function(request,response){
    ...
    client.query(sql,function(error, result, fields){
        if(error){
            ...
            //그냥 데어터 자체를 보내면 문자열에 불과하므로,
            //읽어들인 데이터를 서버에서 해석해서 보내자!!
            //rendering 시켜야 한다!!(ejs 모듈로 렌더링시켜야 함)
...
exports.insert  = function(request,response){
    //post로 전송된 파라미터는 url이 아닌 body로 전송되므로 querystring 모듈로 처리해야함..
    var content="";
...
exports.update  = function(request,response){
    ...
    var writer = urlObj.query.writer;
    ...
    client.query(sql,[title,writer,content,board_id],function(err,fields){
        // response.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
        if(err){
            console.log(err);
            response.end("실패");
